...
class BidsOfBidderActor(val bidder: Bidder) extends Actor with ActorLogging {
  ...
  override def receive = {
    case BidsOfBidderRequest(bidder, auctions, respondTo) => {
      ...
      context.become(
        waitingForGetBidders(sender, respondTo, auctions.map(_._1).toSet))
    }
  }
  ...
  private def waitingForGetBidders(sender: ActorRef,
                                   ...
                                   res: List[BidsOfBidderInOneAuction] =
                                     List.empty): Receive = {
    case GetBidsOfBidderRequestAnswer(answer) =>
      answer match {
        case Left(bidder) => {
          val auctionKey = AuctionKey(auctionId = bidder.auctionId,
                                      auctioneerId = bidder.auctioneerId)
          handleGetBiddersBidAnswer(respondTo,
                                    expectedAnswers - auctionKey,
                                    bidder :: res)
        }
        case Right(notFound) =>
          sender != DeleteFromCache(notFound)
          val auctionKey = AuctionKey(auctionId = notFound.auctionId,
  ...
  private def handleGetBiddersBidAnswer(respondTo: ActorRef,
                                        expectedAnswers: Set[AuctionKey],
                                        res: List[BidsOfBidderInOneAuction]) = {
    expectedAnswers.isEmpty match {
      ...
      case false => {
        context.become(
          waitingForGetBidders(sender, respondTo, expectedAnswers, res))
      }
    }
  }
}
